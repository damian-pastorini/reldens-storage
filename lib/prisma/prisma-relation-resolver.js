...
class PrismaRelationResolver
{
    ...
    hasValidIds(relationData)
    {
        if(!sc.isArray(relationData)){
            return sc.hasOwn(relationData, 'id') && !this.typeCaster.isNullOrEmpty(relationData.id);
        }
        for(let item of relationData){
            if(!sc.hasOwn(item, 'id') || this.typeCaster.isNullOrEmpty(item.id)){
                return false;
            }
        }
        return true;
    ...
    buildCreateDataWithRelations(params, relations)
    {
        ...
        for(let relation of relations){
            ...
            if(this.hasValidIds(relationData)){
                if(sc.isArray(relationData)){
            ...
            if(sc.isArray(relationData)){
                createData[prismaName] = {
                    create: relationData.map(item => this.typeCaster.castFieldsInData(item))
                };
                continue;
            }
            createData[prismaName] = {
                create: this.typeCaster.castFieldsInData(relationData)
            };
        }
        return createData;
    }
