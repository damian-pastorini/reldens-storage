...
class PrismaDriver extends BaseDriver
{
    ...
    buildRelationAliases()
    {
        ...
        for(let prismaRelation of Object.keys(this.relationMetadata)){
            ...
            let aliases = this.generateRelationAliases(prismaRelation, relatedModel, tableName);
    ...
    processFilterValue(value, key)
    {
        if(sc.isArray(value)){
            if('id' === key){
                return value.map(id => this.castToIdType(id)).filter(id => null !== id);
            }
            let fieldType = this.fieldTypes[key];
            if(fieldType){
    ...
    async createWithRelations(params, relations)
    {
        ...
        if(0 < relations.length){
            ...
            try {
                return this.transformRelationResults(
    ...
    async loadByIds(ids)
    {
        let castedIds = ids.map(id => this.castToIdType(id)).filter(id => null !== id);
    ...
    buildIncludeObjectWithMapping(relations)
    {
        ...
        for(let relation of relations){
            ...
            if(!sc.hasOwn(this.relationMetadata, normalizedRelation)){
                //Logger.debug('Relation "'+relation+'" ("'+normalizedRelation+'") not found in '+this.tableName());
                continue;
            }
    ...
    buildNestedIncludeForModel(relationPath, include, mapping, modelName)
    {
        ...
        let modelInfo = dmmf.models?.[modelName.toLowerCase()] || dmmf.models?.find(m => m.name.toLowerCase() === modelName.toLowerCase());
        if(!modelInfo){
            return;
        }
        let relationFields = {};
