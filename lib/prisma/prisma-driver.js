...
class PrismaDriver extends BaseDriver
{
    ...
    isJsonFieldByName(fieldName)
    {
        ...
        let property = this.rawModel.properties[fieldName];
        return property && 'json' === property.dbType;
    }
    ...
    castValue(value, fieldType, fieldName = null)
    {
        ...
        if(this.isJsonFieldType(fieldType)){
            return this.castToJsonType(value);
        }
    ...
    castToIdType(value)
    {
        ...
        if(this.isNumericFieldType(this.idFieldType)){
            let numericId = Number(value);
            if(!isNaN(numericId)){
                return ('Int' === this.idFieldType || 'BigInt' === this.idFieldType) ? Math.floor(numericId) : numericId;
            }
        }
        return String(value);
    }
