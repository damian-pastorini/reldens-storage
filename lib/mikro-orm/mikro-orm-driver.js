...
class MikroOrmDriver extends BaseDriver
{
    ...
    constructor(props)
    {
        ...
        let entityForRepository = this.rawModel;
        if(sc.isObject(this.rawModel) && sc.hasOwn(this.rawModel, 'entity')){
            entityForRepository = this.rawModel.entity;
        }
        this.repository = this.orm.em.getRepository(entityForRepository);
    ...
    getAllRelations()
    {
        if(!this.rawModel.meta || !this.rawModel.meta.properties){
            return [];
        }
        let relations = [];
        for(let propName of Object.keys(this.rawModel.meta.properties)){
            let prop = this.rawModel.meta.properties[propName];
    ...
    async countWithRelations(filters, relations)
    {
        ...
        for(let relationName of relations){
            ...
            if(!relationDriver || !relationDriver.rawModel){
                continue;
            }
            ...
            if('1:m' === prop.kind && prop.mappedBy){
                let mappedProp = relationDriver.rawModel.meta.properties[prop.mappedBy];
